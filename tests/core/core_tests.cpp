...
static std::string checksum_type_as_string(bgcode_EChecksumType type) {
  switch (type) {
...
static std::string block_type_as_string(bgcode_EBlockType type) {
  switch (type) {
...
static std::string compression_type_as_string(bgcode_ECompressionType type) {
  switch (type) {
...
static std::string
metadata_encoding_as_string(bgcode_EMetadataEncodingType type) {
  switch (type) {
  case bgcode_EMetadataEncodingType_INI: {
    return "INI";
  }
  }
  return "";
...
static std::string gcode_encoding_as_string(bgcode_EGCodeEncodingType type) {
  switch (type) {
  case bgcode_EGCodeEncodingType_None: {
    return "None";
  }
  case bgcode_EGCodeEncodingType_MeatPack: {
    return "MeatPack";
  }
  case bgcode_EGCodeEncodingType_MeatPackComments: {
...
static std::string thumbnail_format_as_string(bgcode_EThumbnailFormat type) {
  switch (type) {
  case bgcode_EThumbnailFormat_JPG: {
    return "JPG";
  }
  case bgcode_EThumbnailFormat_PNG: {
    return "PNG";
  }
  case bgcode_EThumbnailFormat_QOI: {
    return "QOI";
  }
  }
  return "";
...
TEST_CASE("File transversal", "[Core]") {
  ...
  REQUIRE(is_valid_binary_gcode(*file, true, checksum_verify_buffer,
                                sizeof(checksum_verify_buffer)) ==
  ...
  REQUIRE(read_header(*file, file_header, nullptr) == bgcode_EResult_Success);
  std::cout << "Checksum type: "
            << checksum_type_as_string(
                   (bgcode_EChecksumType)file_header.checksum_type)
            << "\n";
  ...
  do {
    // read block header
    REQUIRE(read_next_block_header(
                *file, file_header, block_header, checksum_verify_buffer,
                sizeof(checksum_verify_buffer)) == bgcode_EResult_Success);
    std::cout << "Block: "
              << block_type_as_string((bgcode_EBlockType)block_header.type);
    std::cout << " - compression: "
              << compression_type_as_string(
                     (bgcode_ECompressionType)block_header.compression);
    switch ((bgcode_EBlockType)block_header.type) {
    case bgcode_EBlockType_FileMetadata:
    case bgcode_EBlockType_PrinterMetadata:
    case bgcode_EBlockType_PrintMetadata:
    case bgcode_EBlockType_SlicerMetadata: {
      ...
      std::cout << " - encoding: "
                << metadata_encoding_as_string(
                       (bgcode_EMetadataEncodingType)encoding);
      break;
    }
    case bgcode_EBlockType_GCode: {
      const long curr_pos = ftell(file);
      ...
      fseek(file, curr_pos, SEEK_SET);
      std::cout << " - encoding: "
                << gcode_encoding_as_string(
                       (bgcode_EGCodeEncodingType)encoding);
      break;
    }
    case bgcode_EBlockType_Thumbnail: {
      const long curr_pos = ftell(file);
      ThumbnailParams thumbnail_params;
      REQUIRE(thumbnail_params.read(*file) == bgcode_EResult_Success);
      fseek(file, curr_pos, SEEK_SET);
      std::cout << " - format: "
                << thumbnail_format_as_string(
                       (bgcode_EThumbnailFormat)thumbnail_params.format);
      std::cout << " (size: " << thumbnail_params.width << "x"
...
TEST_CASE("Search for GCode blocks", "[Core]") {
  ...
  do {
    ...
    REQUIRE(read_next_block_header(
                *file, file_header, block_header, bgcode_EBlockType_GCode,
                checksum_verify_buffer,
                sizeof(checksum_verify_buffer)) == bgcode_EResult_Success);
    std::cout << "Block type: "
              << block_type_as_string((bgcode_EBlockType)block_header.type)
              << "\n";
